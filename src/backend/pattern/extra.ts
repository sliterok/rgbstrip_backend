...
export const getHeartbeatColor: IColorGetter = (_, time) => {
	...
	const phaseTime = heartbeatPhase * heartbeatCycle
	const fade = Math.min(phaseTime / 200, 1)
	const baseColor = prevHeartbeat.map((c, i) => c + (nextHeartbeat[i] - c) * fade) as IArrColor

	const first = pulseIntensity(phaseTime, 0, heartbeatCycle)
	const second = pulseIntensity(phaseTime, heartbeatCycle / 4, heartbeatCycle)
...
export const getStrobeColor: IColorGetter = (_, time) => {
	...
	const phaseTime = strobePhase * strobeCycle
	const flash = settings.syncToMusic ? strobeCycle * 0.2 : 40
	return phaseTime < flash ? strobeColor : [0, 0, 0]
}
